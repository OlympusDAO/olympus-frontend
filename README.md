...
This has since been improved by adding Treasury Dash middleware that handles combining results and pagination server-side. The complete data set is then retrieved using react-query and a [generated client](https://github.com/OlympusDAO/treasury-subgraph), `@olympusdao/treasury-subgraph-client`.

### Treasury Dash Node URL

Please set the VITE_TREASURY_DASH_PUBLIC_NODE_URL to the relevant treasury dashboard API endpoint
