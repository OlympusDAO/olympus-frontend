...
export const changeStream = createAsyncThunk(
	...
	async({ action, value, recipient, provider, address, networkID }: IActionValueRecipientAsyncThunk, { dispatch }) => {
		...
		try {
			...
			dispatch(fetchPendingTxns({ txHash: streamTx.hash, text: getstreamingTypeText(action), type: pendingTxnType }));
			await streamTx.wait();
		} catch (e: unknown) {
			...
			if (rpcError.code === -32603 && rpcError.message.indexOf("ds-math-sub-underflow") >= 0) {
				dispatch(
					error("You may be trying to stream more than your balance! Error code: 32603. Message: ds-math-sub-underflow"),
				);
			} else {
