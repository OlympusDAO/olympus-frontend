...
export const useBondData = (bond: Bond) => {
  ...
  const key = allBondDataQueryKey(bond.id);
  const useDep = createDependentQuery(key);
  const terms = useDep("terms", () => contract.terms(bond.id));
  const markets = useDep("markets", () => contract.markets(bond.id));
  const quoteTokenPerBaseToken = useDep("quoteTokenPerBaseToken", () =>
  ...
  return useQuery(
    ...
    {
      enabled: !!baseTokenPerUsd && !!quoteTokenPerUsd && !!terms && !!markets && !!quoteTokenPerBaseToken,
    },
  );
};
