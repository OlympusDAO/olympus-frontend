...
export const useClaimRewards = () => {
  ...
  return useMutation<ContractReceipt, EthersError, ClaimRewardsParams>({
    mutationFn: async ({ epochStartDates, amounts, proofs, asVaultToken }) => {
      try {
        ...
        // Validate inputs
        if (epochStartDates.length === 0) throw new Error("No epochs specified for claim");
        if (epochStartDates.length !== amounts.length || epochStartDates.length !== proofs.length) {
        ...
        const transaction = await contract.claim(epochStartDates, amountsBigNumber, proofs, asVaultToken);
