...
import { LibChainId, useGETRewardsEpochRewards } from "src/generated/olympusUnits";
...
export const RewardsStats = () => {
  ...
  // Fetch epoch rewards data
  const { data: epochData } = useGETRewardsEpochRewards({
    chainId: LibChainId.NUMBER_11155111, // Sepolia testnet
  });

  const targetDate = useMemo(() => {
    if (epochData?.endDate) {
      return new Date(epochData.endDate);
    }
    return new Date();
  }, [epochData?.endDate]);
  ...
  const totalDrachmas = useMemo(() => {
    if (!epochData?.rewards) return 0;
    return epochData.rewards.reduce((sum, reward) => {
      return sum + parseFloat(reward.totalUnits || "0");
    }, 0);
  }, [epochData?.rewards]);
