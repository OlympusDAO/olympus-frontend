...
export const UserRewards = () => {
  ...
  // Get all epochs from history with their start dates
  const epochData = useMemo(
    () =>
      userHistoryData?.rewards?.entries?.map(entry => ({
        epochId: entry.epochId,
        // startDate is already in Unix timestamp (seconds) from API
        epochStartDate: entry.startDate,
      })) || [],
  ...
  const contracts = useMemo(
    () =>
      epochData.map(({ epochStartDate }) => ({
        address: contractAddress as `0x${string}`,
        abi: DepositRewardsDistributorABI,
        functionName: "hasClaimed",
        args: [address, epochStartDate],
      })),
  ...
  const claimStatuses = useMemo(() => {
    ...
    return epochData.map((epoch, index) => {
      ...
      return {
        ...
        epochStartDate: epoch.epochStartDate,
        claimed,
      };
    });
    ...
    >
      <Box sx={{ height: "100%", display: "flex", flexDirection: "column" }}>
        ...
        <ClaimRewardsModal
          ...
          epochStartDates={unclaimedEntries.map(entry => entry.startDate)}
