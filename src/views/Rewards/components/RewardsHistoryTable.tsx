...
import { LibChainId, useGETUsersUserHistory } from "src/generated/olympusUnits";
...
const useHasClaimed = (userAddress: string | undefined, epochStartDate: number) => {
  ...
  return useQuery({
    queryKey: ["hasClaimed", userAddress, epochStartDate, networkId],
    queryFn: async () => {
      if (!contract || !userAddress) return false;
      const result = await contract.hasClaimed(userAddress, epochStartDate);
...
const ClaimButton = ({
  ...
  const { data: hasClaimed = false, isLoading: isCheckingClaim } = useHasClaimed(userAddress, epochStartDate);
  ...
  return (
    <>
      ...
      <ClaimRewardsModal
        ...
        epochStartDates={[epochStartDate]}
...
export const RewardsHistoryTable = () => {
  ...
  const { data: userHistoryData, isLoading } = useGETUsersUserHistory(
    address || "",
    {
      chainId: LibChainId.NUMBER_11155111, // Sepolia testnet
    },
    {
      query: {
  ...
  const data =
    userHistoryData?.rewards?.entries?.map(entry => ({
      ...
      epochStartDate: entry.startDate, // Unix timestamp from API
                ...
                >
                  <Box display="flex" justifyContent="flex-end">
                    {address && (
                      <ClaimButton
                        epochId={row.epochId}
                        epochStartDate={row.epochStartDate}
                        userAddress={address}
